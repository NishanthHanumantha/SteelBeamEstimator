...
def _check_sketch_not_found(blocks: List[dict], marks: Tuple[str, ...]) -> List[str]:
    violations: List[str] = []
    block_map = {block["beam_mark"]: block for block in blocks}
    for mark in marks:
        block = block_map.get(mark)
        if block is None:
            violations.append(f"{mark}: block not found")
        elif block.get("status") != "SKETCH_NOT_FOUND":
            violations.append(
                f"{mark}: expected SKETCH_NOT_FOUND, got {block.get('status')}"
            )
    return violations
...
def validate_detail_blocks(blocks: List[dict]) -> dict:
    ...
    violations.extend(_check_sketch_not_found(blocks, ("B10", "B14", "B18")))
