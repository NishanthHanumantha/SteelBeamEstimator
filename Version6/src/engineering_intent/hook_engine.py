"""Evaluate supplementary hook intent."""
...
class HookEngine:
    """Reconstruct hook continuation intents."""
    ...
    def evaluate(self, context: dict[str, Any], snapshot: dict[str, Any]) -> List[dict[str, Any]]:
        ...
        if int(hook_rule.get("rule_count") or 0) <= 0:
            return candidates
        note_rules = context.get("general_note_rules") or {}
        if not note_rules.get("hook_rules"):
            return candidates
        if not context.get("support_refs"):
            return candidates

        for support_zone in context.get("support_zones") or []:
            intent_key = f"{context.get('bar_id')}::{EngineeringIntentType.SUPPLEMENTARY_HOOK.value}::{support_zone}"
            ...
            candidates.append(
                {
                    ...
                    "general_note_id": "RULE::PROJECT#structural_detailing_rules.hook_rules",
                    ...
                    "engineering_justification": (
                        f"Tension bar {context.get('bar_id')} at {support_zone} requires standard hook "
                        f"per General Notes hook rules."
                    ),
                    "reconstruct": True,
                    "context": context,
                }
