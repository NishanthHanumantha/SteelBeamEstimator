#!/usr/bin/env python3
"""W.12 public/local result-delivery E2E. Does not print secrets."""
...
BASE = os.environ.get("STEEL_W12_BASE", "http://13.127.104.99").rstrip("/")
SIXTH = Path(
    r"C:\Users\nishanth.h\SteelBeamEstimator\Test_Input\6th Set Drawings-Inizio_11-18F"
)
OUT = Path(os.environ.get("STEEL_W12_DOWNLOAD_DIR", r"C:\Users\nishanth.h\AppData\Local\Temp\w12_downloads"))
POLL_S = float(os.environ.get("STEEL_W12_POLL_S", "5"))
TIMEOUT_S = float(os.environ.get("STEEL_W12_TIMEOUT_S", "15000"))
...
def sixth_files() -> dict[str, tuple[str, bytes]]:
    gn = next((SIXTH / "general_notes").glob("*.dxf"))
    fr = next((SIXTH / "framing").glob("*.dxf"))
    re = next((SIXTH / "reinforcement").glob("*.dxf"))
    return {
...
def main() -> int:
    ...
    restore = os.environ.get("STEEL_W12_RESTORE_RUN")
    if restore:
    ...
    files = sixth_files()
    print("SIXTH_FILES", {k: (v[0], len(v[1])) for k, v in files.items()})
    ...
    while time.time() < deadline:
        ...
        if status in {"success", "error"}:
            break
        time.sleep(POLL_S)
    print("FINAL_STATUS", json.dumps({k: last.get(k) for k in (
        "status", "run_id", "result_lifecycle", "download_ready", "result_registered",
        "excel_generated", "excel_exists", "workbook_name", "duration_s", "summary", "hybrid", "error"
    )}, default=str))
    ...
    for i in range(2):
        ...
        if st == 200:
            info.update(validate_xlsx(raw, OUT / f"sixth_{run_id}_{i+1}.xlsx"))
        results.append(info)
