"""Regression fingerprints + production firewall for P2.6.10-B.2."""
...
from PhaseP2610B1_population_generalization.regression import (
    capture_fingerprints,
    compare_fingerprints,
    fingerprint_paths as p2610b1_fingerprint_paths,
    firewall_check as p2610b1_firewall_check,
    prior_phase_unit_ok,
)
...
def fingerprint_paths(engine_root: Path, bundle_paths: Dict[str, Path]) -> Dict[str, Path]:
    ...
    out = p2610b1_fingerprint_paths(engine_root, bundle_paths)
    out.update(
        {
            "p2610b1_results": base / "PhaseP2610B1_population_generalization" / "P2.6.10-B.1_RESULTS.json",
            "p2610b1_summary": base / "PhaseP2610B1_population_generalization" / "validation_summary.json",
...
def firewall_check(version10_root: Path) -> Dict[str, Any]:
    ...
    for path in src.rglob("*.py"):
        ...
        if any(rel.startswith(p) or f"/{p}" in f"/{rel}" for p in PRODUCTION_MODULE_PREFIXES):
            ...
            offenders.append(rel)
    nested = p2610b1_firewall_check(version10_root)
    all_off = sorted(set(offenders) | set(nested.get("offenders") or []))
    return {
        "ok": len(all_off) == 0,
        "offenders": all_off,
        "shadow_writes_production": False,
        "note": "P2.6.10-B.2 writes only isolated shadow artefacts",
    }
