"""Regression fingerprints + production firewall for P2.6.9."""
...
from PhaseP268_evidence_conflict_arbitration.regression import (
    capture_fingerprints,
    compare_fingerprints,
    fingerprint_paths as p268_fingerprint_paths,
    firewall_check as p268_firewall_check,
)
...
def fingerprint_paths(engine_root: Path, bundle_paths: Dict[str, Path]) -> Dict[str, Path]:
    ...
    out = p268_fingerprint_paths(engine_root, bundle_paths)
    out.update(
        {
...
def firewall_check(version10_root: Path) -> Dict[str, Any]:
    ...
    for path in src.rglob("*.py"):
        ...
        if any(rel.startswith(p) or f"/{p}" in f"/{rel}" for p in PRODUCTION_MODULE_PREFIXES):
            ...
            offenders.append(rel)
    nested = p268_firewall_check(version10_root)
    all_off = sorted(set(offenders) | set(nested.get("offenders") or []))
    return {
        "ok": len(all_off) == 0,
        "offenders": all_off,
        "shadow_writes_production": False,
        "note": "P2.6.9 writes only isolated group-interpretation shadow artefacts",
    }
