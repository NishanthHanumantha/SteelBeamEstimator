"""
P2.6.2 orchestrator — Selective Vision Candidate Gate.
    ...
    *,
    ...
    if run_tests:
        ...
        if not unit.get("success"):
            ...
            raise RuntimeError(f"P2.6.2 unit tests failed: {failed}")
    ...
    if not fw.get("ok"):
        raise RuntimeError(f"P2.6.2 firewall offenders: {fw.get('offenders')}")
    if not leak.get("ok"):
        raise RuntimeError(f"P2.6.2 runtime leakage: {leak.get('hits')}")
    ...
    for row in regions:
        ...
        decision = build_gate_decision(
            beam_id=beam_id,
            region_id=row.get("region_id") or f"P262::{set_key}::{beam_id}",
            rec=rec,
            model=model,
            set_key=set_key,
    ...
    review_path = out_root / "independent_review.json"
    independent_review = {"status": "PENDING", "blocker": None, "high": None, "medium": None, "low": None}
    ...
    _dump(out_root / "result.json", result)
    m = ev["metrics"]
    _log(
    ...
    if mode == MODE_LIVE:
        _log("  LIVE mode requested but default research path is replay; live not auto-executed.")
    return result


__all__ = ["run_phase_p262"]
