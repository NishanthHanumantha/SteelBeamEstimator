"""Regression fingerprints + production firewall for P2.6.10-E.1."""
...
from PhaseP2610D4_shadow_hybrid_engineering_calculation_accuracy_benchmark.regression import (
    capture_fingerprints,
    compare_fingerprints,
    fingerprint_paths as p2610d4_fingerprint_paths,
    firewall_check as p2610d4_firewall_check,
    prior_phase_unit_ok,
)
...
_SKIP = (
    ...
    "PhaseP2610D3_",
    "PhaseP2610D4_",
    "PhaseP2610E1_", "PhaseP2610E2_", "PhaseP2610E3_",
)
...
def fingerprint_paths(engine_root: Path, bundle_paths: Dict[str, Path]) -> Dict[str, Path]:
    ...
    out = p2610d4_fingerprint_paths(engine_root, bundle_paths)
    out.update(
        {
...
def firewall_check(version10_root: Path) -> Dict[str, Any]:
    ...
    for path in src.rglob("*.py"):
        ...
        if any(rel.startswith(p) or f"/{p}" in f"/{rel}" for p in PRODUCTION_MODULE_PREFIXES):
            ...
            offenders.append(rel)
    nested = p2610d4_firewall_check(version10_root)
    all_off = sorted(set(offenders) | set(nested.get("offenders") or []))
    return {
        "ok": len(all_off) == 0,
        "offenders": all_off,
        "shadow_writes_production": False,
        "note": "P2.6.10-E.1 writes only isolated benchmark artefacts",
    }
