"""Regression fingerprints + production firewall for P2.6.10-D.2."""
...
from PhaseP2610D1_vision_semantic_contract_hybrid_foundation.regression import (
    capture_fingerprints,
    compare_fingerprints,
    fingerprint_paths as p2610d1_fingerprint_paths,
    firewall_check as p2610d1_firewall_check,
    prior_phase_unit_ok,
)
...
_SKIP = (
    ...
    "PhaseP2610C5_",
    "PhaseP2610D1_",
    "PhaseP2610D2_", "PhaseP2610D3_", "PhaseP2610D4_", "PhaseP2610E1_", "PhaseP2610E2_", "PhaseP2610E3_",
)
...
def fingerprint_paths(engine_root: Path, bundle_paths: Dict[str, Path]) -> Dict[str, Path]:
    ...
    out = p2610d1_fingerprint_paths(engine_root, bundle_paths)
    out.update(
        {
            ...
            "p2610d1_population": base / "PhaseP2610D1_vision_semantic_contract_hybrid_foundation" / "benchmark_population_manifest.json",
            "p2610d1_hybrid": base / "PhaseP2610D1_vision_semantic_contract_hybrid_foundation" / "hybrid_semantic_results.json",
        }
    )
    return out
...
def firewall_check(version10_root: Path) -> Dict[str, Any]:
    ...
    for path in src.rglob("*.py"):
        ...
        if any(rel.startswith(p) or f"/{p}" in f"/{rel}" for p in PRODUCTION_MODULE_PREFIXES):
            ...
            offenders.append(rel)
    nested = p2610d1_firewall_check(version10_root)
    all_off = sorted(set(offenders) | set(nested.get("offenders") or []))
    return {
        "ok": len(all_off) == 0,
        "offenders": all_off,
        "shadow_writes_production": False,
        "note": "P2.6.10-D.2 writes only isolated shadow artefacts",
    }
...
def runtime_leakage_scan(package_dir: Path) -> Dict[str, Any]:
    ...
    for name in _RUNTIME_MODULES:
        ...
        for tok in ("anthropic", "ezdxf", "RenderSession", "LIVE_SHADOW", "call_claude"):
            if tok in text:
                hits.append({"file": name, "token": tok})
...
def prior_artefacts_intact(version10_root: Path) -> Dict[str, Any]:
    ...
    needed = [
        ...
        root / "PhaseP2610D1_vision_semantic_contract_hybrid_foundation" / "benchmark_population_manifest.json",
