"""Regression fingerprints + production firewall for P2.6.3."""
...
from PhaseP262_selective_vision_candidate_gate.regression import (
    capture_fingerprints,
    compare_fingerprints,
    fingerprint_paths as p262_fingerprint_paths,
    firewall_check as p262_firewall_check,
)
...
_SKIP = (
    ...
    "PhaseP2610A_",
    "PhaseP2610B_",
    "PhaseP2610B1_", "PhaseP2610B2_", "PhaseP2610B3_", "PhaseP2610C1C2_", "PhaseP2610C3_", "PhaseP2610C4_", "PhaseP2610C5_", "PhaseP2610D1_",
)
...
def fingerprint_paths(engine_root: Path, bundle_paths: Dict[str, Path]) -> Dict[str, Path]:
    ...
    out = p262_fingerprint_paths(engine_root, bundle_paths)
    out.update(
        {
            "p262_status": base / "PhaseP262_selective_vision_candidate_gate" / "P2.6.2_STATUS.md",
            "p262_metrics": base / "PhaseP262_selective_vision_candidate_gate" / "reports" / "metrics.json",
            "p262_manifest": base / "PhaseP262_selective_vision_candidate_gate" / "gate_manifest.json",
        }
    )
    return out
...
def firewall_check(version10_root: Path) -> Dict[str, Any]:
    ...
    for path in src.rglob("*.py"):
        ...
        if any(rel.startswith(p) or f"/{p}" in f"/{rel}" for p in PRODUCTION_MODULE_PREFIXES):
            ...
            offenders.append(rel)
    nested = p262_firewall_check(version10_root)
    ...
    return {
        ...
        "note": "P2.6.3 writes only isolated longitudinal-aware gate shadow artefacts",
    }
