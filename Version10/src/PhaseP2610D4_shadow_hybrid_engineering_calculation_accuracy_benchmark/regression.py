"""Regression fingerprints + production firewall for P2.6.10-D.4."""
...
from PhaseP2610D3_hybrid_engineering_binding_compatibility.regression import (
    capture_fingerprints,
    compare_fingerprints,
    fingerprint_paths as p2610d3_fingerprint_paths,
    firewall_check as p2610d3_firewall_check,
    prior_phase_unit_ok,
)
...
_SKIP = (
    ...
    "PhaseP2610D2_",
    "PhaseP2610D3_",
    "PhaseP2610D4_", "PhaseP2610E1_", "PhaseP2610E2_", "PhaseP2610E3_",
)
...
def fingerprint_paths(engine_root: Path, bundle_paths: Dict[str, Path]) -> Dict[str, Path]:
    ...
    out = p2610d3_fingerprint_paths(engine_root, bundle_paths)
    out.update(
        {
            "p2610d3_results": base / "PhaseP2610D3_hybrid_engineering_binding_compatibility" / "P2.6.10-D.3_RESULTS.json",
            "p2610d3_bindings": base / "PhaseP2610D3_hybrid_engineering_binding_compatibility" / "engineering_binding_results.json",
            "p2610d3_population": base / "PhaseP2610D3_hybrid_engineering_binding_compatibility" / "benchmark_population_manifest.json",
    ...
    for key in ("r13_models", "steel_weight_summary", "bbs_summary"):
...
def firewall_check(version10_root: Path) -> Dict[str, Any]:
    ...
    for path in src.rglob("*.py"):
        ...
        if any(rel.startswith(p) or f"/{p}" in f"/{rel}" for p in PRODUCTION_MODULE_PREFIXES):
            ...
            offenders.append(rel)
    nested = p2610d3_firewall_check(version10_root)
    all_off = sorted(set(offenders) | set(nested.get("offenders") or []))
    return {
        "ok": len(all_off) == 0,
        "offenders": all_off,
        "shadow_writes_production": False,
        "note": "P2.6.10-D.4 writes only isolated shadow artefacts",
    }
...
def prior_artefacts_intact(version10_root: Path) -> Dict[str, Any]:
    ...
    needed = [
        ...
        root / "PhaseP2610D3_hybrid_engineering_binding_compatibility" / "engineering_binding_results.json",
        root / "PhaseP2610D3_hybrid_engineering_binding_compatibility" / "benchmark_population_manifest.json",
