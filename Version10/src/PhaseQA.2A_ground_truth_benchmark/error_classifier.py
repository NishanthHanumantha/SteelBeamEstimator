...
ACCEPTABLE_EXTRA (SPACER_BAR only) is tracked separately and is NOT an error.
"""
...
class ErrorClassifier:
    def classify(
        ...
        acceptable_extra: List[Dict[str, Any]] = []
        for row in bar_matching.get("rows") or []:
            st = row.get("status")
            if st == "ACCEPTABLE_EXTRA":
                # M.2 carve-out — not an error; reported for estimator review
                acceptable_extra.append({
                    "drawing_set": drawing_set,
                    "beam_id": row.get("beam_id"),
                    "bar_role": row.get("bar_role") or row.get("model_role"),
                    "diameter": row.get("model_diameter") or row.get("diameter"),
                    "model_qty": row.get("model_qty"),
                    "detail": (
                        f"SPACER_BAR Y{row.get('model_diameter') or row.get('diameter')} "
                        f"model_qty={row.get('model_qty')} (rule overrides GT omission)"
                    ),
                })
                continue
            if st in status_map:
        ...
        return {
            ...
            "acceptable_extra": acceptable_extra,
            "acceptable_extra_count": len(acceptable_extra),
        }
