"""
P2.1 orchestrator — Leader Tip / Chain Acceptance Analysis.
...
from .population import derive_leader_rows, load_inputs
from .qa_validator import validate_p21
...
class PhaseP21Orchestrator:
    def __init__(
        ...
        self.output_root = (
            ...
            / "data"
            / "output"
            / "PhaseP21_leader_tip_chain_analysis"
        )
    ...
    def run(self) -> Dict[str, Any]:
        print("=" * 72)
        print(f"Phase {PHASE_ID} - Leader Tip / Chain Acceptance Analysis")
        print(f"MODEL_VERSION : {MODEL_VERSION}")
        print("Mode          : DIAGNOSTIC / COUNTERFACTUAL ONLY")
        print("=" * 72)
        ...
        print(
            ...
            f"excluded={population['excluded_count']}"
        )
        ...
        paths = write_all(
            ...
            validation={
                **validation,
                ...
                "root_recommendation": (analysis.get("root_cause") or {})
                .get("recommended_next_phase"),
            },
        )
        elapsed = round(time.perf_counter() - t0, 3)
        rec = (analysis.get("root_cause") or {}).get("recommended_next_phase") or {}
        print(f"\n[{PHASE_ID}] STATUS={validation.get('status')} elapsed={elapsed}s")
        print(f"[{PHASE_ID}] recommendation={rec.get('option')}")
