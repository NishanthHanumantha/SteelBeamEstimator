"""Regression fingerprints + production firewall for P2.6.7."""
...
from PhaseP266_semantic_longitudinal_resolver.regression import (
    capture_fingerprints,
    compare_fingerprints,
    fingerprint_paths as p266_fingerprint_paths,
    firewall_check as p266_firewall_check,
)
...
def fingerprint_paths(engine_root: Path, bundle_paths: Dict[str, Path]) -> Dict[str, Path]:
    ...
    out = p266_fingerprint_paths(engine_root, bundle_paths)
    out.update(
        {
            "p266_status": base / "PhaseP266_semantic_longitudinal_resolver" / "P2.6.6_STATUS.md",
            "p266_metrics": base / "PhaseP266_semantic_longitudinal_resolver" / "P2.6.6_METRICS.json",
...
def firewall_check(version10_root: Path) -> Dict[str, Any]:
    ...
    for path in src.rglob("*.py"):
        ...
        if any(rel.startswith(p) or f"/{p}" in f"/{rel}" for p in PRODUCTION_MODULE_PREFIXES):
            ...
            offenders.append(rel)
    nested = p266_firewall_check(version10_root)
    all_off = sorted(set(offenders) | set(nested.get("offenders") or []))
    return {
        "ok": len(all_off) == 0,
        "offenders": all_off,
        "shadow_writes_production": False,
        "note": "P2.6.7 writes only isolated live-arbitration shadow artefacts",
    }
