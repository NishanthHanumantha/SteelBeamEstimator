"""Regression fingerprints + production firewall for P2.6.10-B.3."""
...
from PhaseP2610B2_render_quality_directional_recovery.regression import (
    capture_fingerprints,
    compare_fingerprints,
    fingerprint_paths as p2610b2_fingerprint_paths,
    firewall_check as p2610b2_firewall_check,
    prior_phase_unit_ok,
)
...
_SKIP = (
    ...
    "PhaseP2610B2_",
    "PhaseP2610B3_",
    "PhaseP2610C1C2_", "PhaseP2610C3_", "PhaseP2610C4_", "PhaseP2610C5_", "PhaseP2610D1_", "PhaseP2610D2_",
)
...
def fingerprint_paths(engine_root: Path, bundle_paths: Dict[str, Path]) -> Dict[str, Path]:
    ...
    out = p2610b2_fingerprint_paths(engine_root, bundle_paths)
    out.update(
        {
            "p2610b2_results": base / "PhaseP2610B2_render_quality_directional_recovery" / "P2.6.10-B.2_RESULTS.json",
            "p2610b2_summary": base / "PhaseP2610B2_render_quality_directional_recovery" / "validation_summary.json",
...
def firewall_check(version10_root: Path) -> Dict[str, Any]:
    ...
    for path in src.rglob("*.py"):
        ...
        if any(rel.startswith(p) or f"/{p}" in f"/{rel}" for p in PRODUCTION_MODULE_PREFIXES):
            ...
            offenders.append(rel)
    nested = p2610b2_firewall_check(version10_root)
    all_off = sorted(set(offenders) | set(nested.get("offenders") or []))
    return {
        "ok": len(all_off) == 0,
        "offenders": all_off,
        "shadow_writes_production": False,
        "note": "P2.6.10-B.3 writes only isolated shadow artefacts",
    }
...
def prior_artefacts_intact(version10_root: Path) -> Dict[str, Any]:
    ...
    needed = [
        ...
        root / "PhaseP2610B_adaptive_beam_detail_crop" / "unit_tests.json",
