"""Regression fingerprints + production firewall for P2.6.10-D.3."""
...
from PhaseP2610D2_shadow_hybrid_semantic_resolver.regression import (
    capture_fingerprints,
    compare_fingerprints,
    fingerprint_paths as p2610d2_fingerprint_paths,
    firewall_check as p2610d2_firewall_check,
    prior_phase_unit_ok,
)
...
_SKIP = (
    ...
    "PhaseP2610D1_",
    "PhaseP2610D2_",
    "PhaseP2610D3_", "PhaseP2610D4_", "PhaseP2610E1_", "PhaseP2610E2_", "PhaseP2610E3_",
)
...
def fingerprint_paths(engine_root: Path, bundle_paths: Dict[str, Path]) -> Dict[str, Path]:
    ...
    out = p2610d2_fingerprint_paths(engine_root, bundle_paths)
    out.update(
        {
            "p2610d2_results": base / "PhaseP2610D2_shadow_hybrid_semantic_resolver" / "P2.6.10-D.2_RESULTS.json",
            "p2610d2_hybrid": base / "PhaseP2610D2_shadow_hybrid_semantic_resolver" / "hybrid_beam_results.json",
            "p2610d2_population": base / "PhaseP2610D2_shadow_hybrid_semantic_resolver" / "benchmark_population_manifest.json",
    ...
    r13 = bundle_paths.get("r13_models") if bundle_paths else None
    if r13:
        out["r13_models"] = Path(r13)
    return out
...
def firewall_check(version10_root: Path) -> Dict[str, Any]:
    ...
    for path in src.rglob("*.py"):
        ...
        if any(rel.startswith(p) or f"/{p}" in f"/{rel}" for p in PRODUCTION_MODULE_PREFIXES):
            ...
            offenders.append(rel)
    nested = p2610d2_firewall_check(version10_root)
    all_off = sorted(set(offenders) | set(nested.get("offenders") or []))
    return {
        "ok": len(all_off) == 0,
        "offenders": all_off,
        "shadow_writes_production": False,
        "note": "P2.6.10-D.3 writes only isolated shadow artefacts",
    }
...
def runtime_leakage_scan(package_dir: Path) -> Dict[str, Any]:
    ...
    for name in _RUNTIME_MODULES:
        ...
        for tok in ("anthropic", "ezdxf", "RenderSession", "LIVE_SHADOW", "call_claude", "LIVE_CLAUDE"):
            if tok in text:
                hits.append({"file": name, "token": tok})
...
def prior_artefacts_intact(version10_root: Path) -> Dict[str, Any]:
    ...
    needed = [
        ...
        root / "PhaseP2610D2_shadow_hybrid_semantic_resolver" / "hybrid_beam_results.json",
        root / "PhaseP2610D2_shadow_hybrid_semantic_resolver" / "benchmark_population_manifest.json",
        root / "PhaseP2610D1_vision_semantic_contract_hybrid_foundation" / "P2.6.10-D.1_RESULTS.json",
        root / "PhaseP2610D1_vision_semantic_contract_hybrid_foundation" / "hybrid_authority_contract.json",
        root / "PhaseP2610C1C2_evidence_inventory_candidate_selection" / "selection_manifest.json",
        root / "PhaseP2610C5_stratified_vision_semantic_benchmark" / "selected_sample.json",
        root / "PhaseP2610C3_visual_completeness_claude_shadow" / "six_beam_benchmark.json",
    ]
