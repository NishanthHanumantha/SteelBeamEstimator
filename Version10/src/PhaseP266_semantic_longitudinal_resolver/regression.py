"""Regression fingerprints + production firewall for P2.6.6."""
...
from PhaseP265_spatial_context_longitudinal.regression import (
    capture_fingerprints,
    compare_fingerprints,
    fingerprint_paths as p265_fingerprint_paths,
    firewall_check as p265_firewall_check,
)
...
def fingerprint_paths(engine_root: Path, bundle_paths: Dict[str, Path]) -> Dict[str, Path]:
    ...
    out = p265_fingerprint_paths(engine_root, bundle_paths)
    out.update(
        {
            ...
            "p265_metrics": base / "PhaseP265_spatial_context_longitudinal" / "reports" / "metrics.json",
            "p265_manifest": base / "PhaseP265_spatial_context_longitudinal" / "gate_manifest.json",
            "p265_decisions": base / "PhaseP265_spatial_context_longitudinal" / "gate_decisions.json",
        }
    )
    return out
...
def firewall_check(version10_root: Path) -> Dict[str, Any]:
    ...
    for path in src.rglob("*.py"):
        ...
        if any(rel.startswith(p) or f"/{p}" in f"/{rel}" for p in PRODUCTION_MODULE_PREFIXES):
            ...
            offenders.append(rel)
    nested = p265_firewall_check(version10_root)
    ...
    return {
        ...
        "note": "P2.6.6 writes only isolated semantic-resolver shadow artefacts",
    }
