"""
Run context for web / offline pipeline execution (Version9 accuracy branch).

MODEL_VERSION: 8.9.5 (baseline; bump with Version9 accuracy milestones)

Every production stage should resolve paths through RunContext instead of
hardcoding shared data/output or Benchmark_Set_* folders.

Web contract (set by webapp for every stage subprocess):
  STEEL_ENGINE_ROOT  → absolute path to Version9/
  STEEL_RUN_ROOT     → absolute path to Version9/data/web_runs/<run_id>/
  STEEL_OUTPUT_ROOT  → STEEL_RUN_ROOT/data/output

Offline CLI (no STEEL_RUN_ROOT):
  run_root = engine_root  → outputs remain Version9/data/output/...
"""
...
PHASE_R3 = "PhaseR3_geometry_context_engine"
