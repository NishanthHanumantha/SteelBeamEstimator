"""
P2.6.4 CALL / SKIP / HOLD rules.

Stirrup path remains frozen. Non-ROLE_COVERAGE_GAP longitudinal paths remain P2.6.3.
Only ROLE_COVERAGE_GAP uses the selective role-gap overlay.
"""
...
_LONG_SHORTFALL = {COVER_QTY, COVER_MULTI, COVER_MISSING}
...
def _frozen_stirrup_calls(features: Dict[str, Any], call: List[str]) -> None:
    """Exact P2.6.2 / P2.6.3 stirrup CALL conditions. Do not redesign."""
...
def decide_gate(features: Dict[str, Any]) -> Dict[str, Any]:
    ...
    elif cov == COVER_LAYER:
        if features.get("role_gap_status") != ROLE_GAP_EXPLAINED:
    ...
    if call and "OTHER_BEAM_ASSOCIATION" not in hold:
        ...
        strength = "LOW"
        if features.get("role_gap_status") == ROLE_GAP_EXPLAINED:
            skip.append("ROLE_GAP_DETERMINISTICALLY_EXPLAINED")
