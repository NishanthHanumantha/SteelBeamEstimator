"""
P2.6.4 orchestrator — Selective XOR / Role-Gap Refinement.
...
def run_phase_p264(
    *,
    ...
    if mode == MODE_LIVE:
        _log("  LIVE mode requested; P2.6.4 still executes REPLAY_P261_CACHED only.")
        mode = MODE_REPLAY
    ...
    if run_tests:
        ...
        if not unit.get("success"):
            ...
            raise RuntimeError(f"P2.6.4 unit tests failed: {failed}")
    ...
    if not fw.get("ok"):
        raise RuntimeError(f"P2.6.4 firewall offenders: {fw.get('offenders')}")
    if not leak.get("ok"):
        raise RuntimeError(f"P2.6.4 runtime leakage: {leak.get('hits')}")
    ...
    for row in regions:
        ...
        _log(
            ...
            f"rg={decision.get('role_gap_status')} {decision['reason_codes']}"
        )
    ...
    _dump(out_root / "role_gap_diagnostics.json", ev.get("role_gap_diagnostics") or [])
    ...
    result = {
        ...
        "false_skips": ev["false_skips"],
        "false_calls": ev["false_calls"],
        "role_gap_diagnostics": ev.get("role_gap_diagnostics") or [],
        "decisions": decisions,
        "evidence": evidence,
        "unit_tests": unit,
        "firewall": fw,
    ...
    _log(
        f"  role_gap call={m.get('ROLE_COVERAGE_GAP_CALLS')} "
        f"skip={m.get('ROLE_COVERAGE_GAP_SKIPS')} explained={m.get('ROLE_GAP_EXPLAINED_BEAMS')}"
    )
    _log(f"  decision={result.get('decision')} strength={result.get('strength')}")
    return result


__all__ = ["run_phase_p264"]
