...
Phase M.2 carve-out:
  Model SPACER_BAR with no GT match → ACCEPTABLE_EXTRA
  (rule overrides GT omissions; excluded from Extra Bar penalties).
"""
...
class BarMatcher:
    ...
    def match_beam_bars(
        ...
        for i, mb in enumerate(mod_bars):
            if not mod_used[i]:
                # M.2 — SPACER_BAR extras are ACCEPTABLE_EXTRA (rule overrides GT)
                status = (
                    "ACCEPTABLE_EXTRA"
                    if _role_key(mb.bar_role) == _SPACER_ROLE
                    else "EXTRA"
                )
                results.append(self._row(
                    drawing_set, est_beam.beam_id, None, mb, status
                ))

        _non_det = ("MISSING", "EXTRA", "ACCEPTABLE_EXTRA")
        detected = sum(1 for r in results if r["status"] not in _non_det)
        ...
        return {
            ...
            "acceptable_extra_bars": sum(
                1 for r in results if r["status"] == "ACCEPTABLE_EXTRA"
            ),
