...
class ExcelExporter:
    def export(
        ...
        self._acceptable_extra(wb.create_sheet("ACCEPTABLE_EXTRA Spacers"), results)
        self._role_status(wb.create_sheet("Role Status Matrix"), results)
    ...
    def _acceptable_extra(self, ws, results: List[Dict[str, Any]]) -> None:
        """M.2 carve-out — model SPACER_BAR where GT omitted (not an error)."""
        headers = ["Drawing Set", "Beam", "Role", "Diameter", "Model Qty", "Detail"]
        rows = []
        for r in results:
            for e in (r.get("errors") or {}).get("acceptable_extra") or []:
                rows.append([
                    e.get("drawing_set"), e.get("beam_id"), e.get("bar_role"),
                    e.get("diameter"), e.get("model_qty"), e.get("detail"),
                ])
            for e in (r.get("bar_matching") or {}).get("acceptable_extra_detail") or []:
                rows.append([
                    e.get("drawing_set"), e.get("beam_id"),
                    e.get("bar_role") or e.get("model_role"),
                    e.get("model_diameter") or e.get("diameter"),
                    e.get("model_qty"), "ACCEPTABLE_EXTRA",
                ])
        # Deduplicate by first 5 cols
        seen = set()
        unique = []
        for row in rows:
            key = tuple(row[:5])
            ...
            seen.add(key)
            unique.append(row)
        _table(ws, 1, headers, unique)

    def _role_status(self, ws, results: List[Dict[str, Any]]) -> None:
        statuses = [
        ...
        rows = []
        for r in results:
            matrix = (r.get("bar_matching") or {}).get("role_status_matrix") or {}
            for role, counts in sorted(matrix.items()):
                rows.append(
                    [r.get("drawing_set"), role]
