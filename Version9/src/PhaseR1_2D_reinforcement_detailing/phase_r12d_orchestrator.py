...
class PhaseR12DOrchestrator:
    ...
    def run(self) -> Dict[str, Any]:
        print("=" * 72)
        print("Phase R.1.2D - Reinforcement Detailing Interpretation Engine")
        print(f"MODEL_VERSION : {MODEL_VERSION}")
        print("=" * 72)
        t0 = time.perf_counter()

        _load_pkg("PhaseR12D", self._src, [
        ...
        # Load intents via R.1.2C (read-only — do not modify R.1.2C package)
        print("\n[1/7] Loading EngineeringIntents (R.1.2C) ...")
        intents = self._load_intents()
        print(f"      Intents={len(intents)}")

        print("\n[2/7] Building ReinforcementDetails ...")
        Engine = sys.modules[
            "PhaseR12D.reinforcement_detail_engine"
        ].ReinforcementDetailEngine
        ctx = self._load_ctx()
        details, payload = Engine(self.v7, ctx).build_from_intents(intents)
        print(
            f"      Details={len(details)} "
            f"stirrup_segs={len(payload.get('stirrup_segments') or [])} "
            f"conf_mean={(payload.get('confidence') or {}).get('mean')}"
        )

        print("\n[3/7] Rebuilding R.1.3 + V.B.1 with Detail layer ...")
        ...
        mapping = self._build_mapping(details)
        builder_uses_details = self._check_builder_uses_details()
        validators = sys.modules["PhaseR12D.detail_validators"]
        regression = validators.RegressionDetailValidator().validate(
            self.v7, len(details), len(mapping)
        )
        validation = validators.DetailPhaseValidator().validate(
            intents,
            details,
            mapping,
        ...
        recommendation = (
            ...
            else ("C" if validation.get("passed", 0) < 5 else "B")
        )
        ...
        result: Dict[str, Any] = {
            ...
            "builder_uses_details": builder_uses_details,
        }

        print("\n[5/7] Exporting artefacts ...")
        Exporter = sys.modules["PhaseR12D.detail_exporter"].DetailExporter
        ...
        print(f"      Consistency passed: {(payload.get('consistency') or {}).get('passed')}")
    ...
    def _build_mapping(self, details) -> List[Dict[str, Any]]:
        mapping = [
            {
                ...
                "extent": d.extent,
                "curtailment_type": d.curtailment_type,
    ...
    def _check_builder_uses_details(self) -> bool:
        path = (
            self.v7
        ...
        return "_bars_from_details" in text and "_resolve_details" in text
