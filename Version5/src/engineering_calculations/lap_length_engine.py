"""Lap length determination engine — Phase I.5."""
...
from src.engineering_calculations.lap_length_determiner import LapLengthDeterminer
...
class LapLengthEngine:
    """Determine lap length for READY LAP_LENGTH calculation results."""
    ...
    def determine(
        ...
        registry = LapLengthRegistry()
        determiner = LapLengthDeterminer(
            self._cache,
        ...
        for result in results:
            if result.get("calculation_type") != CalculationType.LAP_LENGTH.value:
                continue
            ...
            lap_length_id = registry.next_id()
            record["lap_length_id"] = lap_length_id
        ...
        project_registry = LapLengthRegistry.build_project_registry(
            lap_records,
        ...
        exports = {
            ...
            "lap_length_results": lap_records,
            "lap_length_registry": project_registry,
        }
    ...
    def build_project_exports(
        results: List[dict[str, Any]],
        lap_records: List[dict[str, Any]],
        lap_registry: dict[str, Any],
        calc_registry: dict[str, Any],
    ) -> dict[str, Any]:
        synced_registry = LapLengthEngine.sync_calculation_result_registry(
            results,
            calc_registry,
        )
        return {
            ...
            "lap_length_results": lap_records,
            "lap_length_registry": lap_registry,
        }
