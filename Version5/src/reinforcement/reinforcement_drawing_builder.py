...
from src.reinforcement_calculation.reinforcement_readiness_validator import (
    ReinforcementReadinessValidator,
)
from src.reinforcement_calculation.reinforcement_reporting import (
    ReinforcementReadinessReporting,
    ReinforcementReporting,
)
from src.reinforcement_calculation.reinforcement_summary import ReinforcementSummary
from src.reinforcement_calculation.reinforcement_validator import ReinforcementValidator
from src.calculation_context.context_reporting import CalculationContextReporting
from src.calculation_context.context_summary import CalculationContextSummary
from src.calculation_context.context_validator import CalculationContextValidator
...
from src.engineering_geometry.geometry_validator import GeometryAssociationValidator
from src.engineering_objects.engineering_object_builder import EngineeringObjectBuilder
from src.engineering_specifications.reference_contract import build_reference_contract
...
from src.engineering_specifications.specification_reference_validator import (
    SpecificationReferenceValidator,
)
from src.engineering_specifications.specification_summary import SpecificationSummary
from src.engineering_specifications.specification_validator import SpecificationValidator
...
class ReinforcementDrawingBuilder:
    ...
    def __init__(self, config: dict[str, Any]) -> None:
        ...
        self._engineering_specification_enabled = bool(
            g2.get("engineering_specification_enable", True)
        )
        self._engineering_specification_validation_enabled = bool(
            g2.get("engineering_specification_validation_enable", True)
        )
        self._geometry_association_enabled = bool(
            g2.get("geometry_association_enable", True)
        )
        self._geometry_association_validation_enabled = bool(
            g2.get("geometry_association_validation_enable", True)
        )
        self._calculation_context_enabled = bool(
            g2.get("calculation_context_enable", True)
        )
        self._calculation_context_validation_enabled = bool(
            g2.get("calculation_context_validation_enable", True)
        )
        self._reinforcement_calculation_enabled = bool(
            g2.get("reinforcement_calculation_enable", True)
        )
        self._reinforcement_calculation_validation_enabled = bool(
            g2.get("reinforcement_calculation_validation_enable", True)
        )
        self._reinforcement_readiness_enabled = bool(
            g2.get("reinforcement_readiness_enable", True)
        )
    ...
    def build_model(self, model: dict[str, Any]) -> dict[str, Any]:
        ...
        if self._engineering_specification_enabled and model.get(
            "resolved_engineering_properties"
        ):
            project_id = str(model.get("project_workspace", {}).get("project_id", ""))
            role_list = model.get("engineering_semantic_role_registry", {}).get("roles", [])
            spec_builder = SpecificationBuilder()
            specifications, spec_registry = spec_builder.build(
            ...
            h1_exports = SpecificationBuilder.build_project_exports(
                specifications,
                spec_registry,
                model.get("engineering_objects", []),
                drawing_models,
            ...
            model.update(h1_exports)
            model["specification_summary"] = SpecificationSummary.build(
                model.get("engineering_objects", []),
                specifications,
                model.get("specification_registry", {}),
                {"status": "SKIP", "summary": {}},
            )

        if self._geometry_association_enabled and model.get("engineering_specifications"):
            project_id = str(model.get("project_workspace", {}).get("project_id", ""))
            ga_builder = GeometryAssociationBuilder()
            associations, ga_registry = ga_builder.build(
                model.get("engineering_specifications", []),
                model,
            )
            h2_exports = GeometryAssociationBuilder.build_project_exports(
                associations,
                ga_registry,
                model.get("engineering_specifications", []),
                drawing_models,
        ...
        if self._calculation_context_enabled and model.get("geometry_associations"):
            project_id = str(model.get("project_workspace", {}).get("project_id", ""))
            cc_builder = CalculationContextBuilder()
            contexts, cc_registry = cc_builder.build(
                model.get("engineering_specifications", []),
                model.get("geometry_associations", []),
                model,
                drawing_models,
            )
            i1_exports = CalculationContextBuilder.build_project_exports(
                contexts,
                cc_registry,
                model.get("engineering_specifications", []),
                drawing_models,
            ...
            model.update(i1_exports)
            model["calculation_context_summary"] = CalculationContextSummary.build(
                model.get("engineering_specifications", []),
                contexts,
                model.get("calculation_context_registry", {}),
                {"status": "SKIP", "summary": {}},
            )

        if self._reinforcement_calculation_enabled and model.get("calculation_contexts"):
            project_id = str(model.get("project_workspace", {}).get("project_id", ""))
            rb_builder = ReinforcementBuilder()
            bars, groups, rb_registry = rb_builder.build(
                model.get("engineering_specifications", []),
            ...
            model.update(i2_exports)
            model["reinforcement_summary"] = ReinforcementSummary.build(
                model.get("engineering_specifications", []),
                bars,
                groups,
                model.get("reinforcement_registry", {}),
                {"status": "SKIP", "summary": {}},
            )
        ...
        g_h1_spec_validation: dict[str, Any] = {"status": "SKIP", "checks": [], "summary": {}}
        if model.get("specification_registry") and model.get("engineering_specifications") is not None:
            g_h1_spec_validation = SpecificationValidator().validate(model)
            SpecificationReporting.apply_validation(model, g_h1_spec_validation)
        else:
            model["specification_validation"] = g_h1_spec_validation

        g_h11_reference_validation: dict[str, Any] = {
            "status": "SKIP",
            "checks": [],
            "summary": {},
        }
        if model.get("engineering_specifications") is not None:
            g_h11_reference_validation = SpecificationReferenceValidator().validate(model)
            model["specification_reference_validation"] = g_h11_reference_validation
            model["reference_contract"] = build_reference_contract()
        else:
            model["specification_reference_validation"] = g_h11_reference_validation

        g_h2_geometry_validation: dict[str, Any] = {"status": "SKIP", "checks": [], "summary": {}}
        if model.get("geometry_registry") and model.get("geometry_associations") is not None:
            g_h2_geometry_validation = GeometryAssociationValidator().validate(model)
            GeometryAssociationReporting.apply_validation(model, g_h2_geometry_validation)
        else:
            model["geometry_validation"] = g_h2_geometry_validation

        g_i1_context_validation: dict[str, Any] = {"status": "SKIP", "checks": [], "summary": {}}
        if model.get("calculation_context_registry") and model.get("calculation_contexts") is not None:
            g_i1_context_validation = CalculationContextValidator().validate(model)
            CalculationContextReporting.apply_validation(model, g_i1_context_validation)
        else:
            model["calculation_context_validation"] = g_i1_context_validation

        g_i2_reinforcement_validation: dict[str, Any] = {"status": "SKIP", "checks": [], "summary": {}}
        if model.get("reinforcement_registry") and model.get("reinforcement_groups") is not None:
            g_i2_reinforcement_validation = ReinforcementValidator().validate(model)
            ReinforcementReporting.apply_validation(model, g_i2_reinforcement_validation)
        else:
            model["reinforcement_validation"] = g_i2_reinforcement_validation

        g_i21_readiness_validation: dict[str, Any] = {"status": "SKIP", "checks": [], "summary": {}}
        if (
            self._reinforcement_readiness_enabled
            and model.get("reinforcement_registry")
            and model.get("reinforcement_groups") is not None
        ):
            model["reinforcement_readiness"] = ReinforcementExporter.export_readiness(
                model.get("reinforcement_bars", []),
                model.get("reinforcement_groups", []),
            )
            if self._reinforcement_readiness_validation_enabled:
                g_i21_readiness_validation = ReinforcementReadinessValidator().validate(model)
                ReinforcementReadinessReporting.apply_validation(
                    model,
                    g_i21_readiness_validation,
                )
            else:
        ...
        manager["engineering_specification_complete"] = (
            model.get("specification_validation", {}).get("status") == "PASS"
            or model.get("specification_validation", {}).get("status") == "SKIP"
        )
        manager["geometry_association_complete"] = (
            model.get("geometry_validation", {}).get("status") == "PASS"
            or model.get("geometry_validation", {}).get("status") == "SKIP"
        )
        manager["calculation_context_complete"] = (
            model.get("calculation_context_validation", {}).get("status") == "PASS"
            or model.get("calculation_context_validation", {}).get("status") == "SKIP"
        )
        manager["reinforcement_calculation_complete"] = (
            model.get("reinforcement_validation", {}).get("status") == "PASS"
            or model.get("reinforcement_validation", {}).get("status") == "SKIP"
        )
