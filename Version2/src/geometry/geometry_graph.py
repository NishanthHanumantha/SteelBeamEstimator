...
class GeometryGraphBuilder:
    ...
    def build_sketch(
        ...
        sketches = self.build_sketches(doc, label_x, label_y)
    ...
    def build_sketches(
        ...
        if not seed_polys:
            logger.warning(
                "No seed polylines near label at ({}, {})",
                label_x,
                label_y,
            )
            return []
        ...
        for seed_poly in seed_polys:
            x_column = self._x_column_bbox([seed_poly], label_x)
            ...
            component_segments = tuple(
                segments[index] for index in component_indices
            )
            bbox = _bbox_from_segments(component_segments)
            bbox_key = tuple(round(value, 0) for value in bbox)
            if bbox_key in seen_bboxes:
                continue
            seen_bboxes.add(bbox_key)

            logger.debug(
                "Sketch at ({}, {}) seed {}: {} segments, bbox={}",
                label_x,
                label_y,
                seed_poly[4],
            ...
            sketches.append(
                SketchComponent(
                    ...
                    bbox=bbox,
                    seed_polyline_handle=str(seed_poly[4]),
                    x_column=x_column,
                )
            )
    ...
    def build_merged_sketch(
        self,
        doc: Drawing,
        label_x: float,
        label_y: float,
    ) -> SketchComponent | None:
        """Flood-fill across all seed polylines (legacy single-sketch path)."""
        segments, polyline_bboxes = self._extract_segments(doc)
        if not segments:
            return None
        ...
        bbox = _bbox_from_segments(component_segments)
        seed_handle = seed_polys[0][4] if seed_polys else ""

        return SketchComponent(
            segments=component_segments,
            bbox=bbox,
            seed_polyline_handle=str(seed_handle),
            x_column=x_column,
        )
    ...
    def _select_seed_polylines(
        ...
        for bbox in polyline_bboxes:
            ...
            layer = bbox[5] if len(bbox) > 5 else "-STR-BEAM"
            width = xmax - xmin
            height = ymax - ymin
            ...
            if width < 80.0 or height < 80.0:
                continue
            max_seed_width = MAX_SEED_POLYLINE_WIDTH_MM.get(layer, 900.0)
