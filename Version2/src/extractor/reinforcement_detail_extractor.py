...
class ReinforcementDetailExtractor:
    ...
    def extract(
        ...
        for header in headers:
            ...
            sketches = self._geometry_builder.build_sketches(
                doc, header["x"], header["y"]
            )
            valid_sketches = [
                sketch
                for sketch in sketches
                if validate_sketch(sketch, label_y=header["y"])[0]
            ]

            if not valid_sketches:
                merged = self._geometry_builder.build_merged_sketch(
                    doc, header["x"], header["y"]
                )
                if merged and validate_sketch(merged, label_y=header["y"])[0]:
                    valid_sketches = [merged]
                elif merged:
                    logger.warning(
                        ...
                        header["x"],
                        header["y"],
                        validate_sketch(merged, label_y=header["y"])[1],
                    )
                    continue
                else:
                    logger.warning(
                        "No sketch geometry for {} at ({}, {})",
                        mark,
                        header["x"],
                        header["y"],
                    )
            ...
            for sketch_component in valid_sketches:
                expanded = expand_sketch_bbox(sketch_component.bbox)
